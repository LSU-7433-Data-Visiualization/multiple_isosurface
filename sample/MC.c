	...
	for(i = 0; i < maxX - inc; i += inc) {
		for(j = 0; j < maxY - inc; j += inc) {
			for(k = 0; k < maxZ - inc; k += inc) {
				...
				v.p[0].x = i * 1.0; v.p[0].y = j * 1.0; v.p[0].z = k * 1.0; 

				v.val[1] = dataset[i+inc][j][k];
				v.p[1].x = i + inc * 1.0; v.p[1].y = j * 1.0; v.p[1].z = k * 1.0; 

				v.val[2] = dataset[i+inc][j+inc][k];
				v.p[2].x = i + inc * 1.0; v.p[2].y = j + inc * 1.0; v.p[2].z = k * 1.0; 

				v.val[3] = dataset[i][j+inc][k];
				v.p[3].x = i * 1.0; v.p[3].y = j + inc * 1.0; v.p[3].z = k * 1.0; 

				v.val[4] = dataset[i][j][k+inc];
				v.p[4].x = i * 1.0; v.p[4].y = j * 1.0; v.p[4].z = k + inc * 1.0; 

				v.val[5] = dataset[i+inc][j][k+inc];
				v.p[5].x = i + inc * 1.0; v.p[5].y = j * 1.0; v.p[5].z = k + inc * 1.0; 

				v.val[6] = dataset[i+inc][j+inc][k+inc];
				v.p[6].x = i + inc * 1.0; v.p[6].y = j + inc * 1.0; v.p[6].z = k + inc * 1.0; 

				v.val[7] = dataset[i][j+inc][k+inc];
				v.p[7].x = i * 1.0; v.p[7].y = j + inc * 1.0; v.p[7].z = k + inc * 1.0; 
